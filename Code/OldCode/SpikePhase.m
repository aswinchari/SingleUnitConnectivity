...
for a = 1:length(filelist)
    ...
    for b = 1:length(allclusters)
        ...
        subplot(length(allclusters),2,2*b-1)
        imagesc(tw*1e3,allclusters(b).aw, allclusters(b).WSH');
        title(strcat('Channel',allclusters(b).channel(25:end),' | Neuron',num2str(allclusters(b).cluster)))
        
        subplot(length(allclusters),2,2*b)
        bar(allclusters(b).ts,allclusters(b).scount)
        title(strcat('Refractory =',num2str(round(allclusters(b).refrac*100,1)),'%'))
    
    end
    
    % saveas(gcf,'Figures/Spikes.png');
