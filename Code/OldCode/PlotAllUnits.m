%% Plot all units from all times_*.mat files for single patient
...
filelist = dir('Data/WaveClus*');

%% Go make plots for each unit

for a = 1:length(filelist)
    
    cd(strcat(filelist(a).folder,'/',filelist(a).name))

    filelist2 = dir('times*')
    
    for b = 1:length(filelist2)
        
        CurFile = filelist2(b).name;
        ...
        for c = 1:length(cluslabels)
            ...
            % save plot here
            saveas(gcf,join([(CurFile(1:end-4)),'unit',string(c),'.jpg']))
